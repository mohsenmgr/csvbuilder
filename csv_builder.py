...
class CsvBuilder:
    ...
    def __init__(self,diviceIdentifierColumn,excelfile,sheet1,sheet2,desiredColumns) -> None:
        ...
        self.sheet1Columns = []
        self.sheet2Columns = []
        self.dataFrame1 = pd.read_excel(self.xlsFile,sheet_name=self.sheet1Name)
        self.dataFrame2 = pd.read_excel(self.xlsFile,sheet_name=self.sheet2Name)
    ...
    def change_order(self , data: dict,output_columns:[]) -> dict:
        ...
        sorted_tuple_list = sorted(data.items(), key=lambda item: output_columns.index(item[0]))
    ...
    def create_output_device_list(self) -> None:
        ...
        for value in excel_dataframe_df[column_name]:
            ...
            if previous_value is not None and value != previous_value:
                number_of_files +=1
                self.device_name_list.append((previous_value,row_number_start,row_number_end))
    ...
    def fill_appendix_data(self) -> None:
        for device in self.device_name_list:
            ...
            if len(self.appendixData[deviceName]) > 0:
                res = self.check_column_header_against_result_column_header(self.appendixData[deviceName][0].keys(),self.desiredColumns)
                #print(self.appendixData[device][0].keys())

                if len(res) > 0:
                    print(f"different column name/s found in sheet {deviceName}, columns: {res}")
                    exit()
    ...
    def combiner(self) -> None:
        sheet2TotalRows = builder.get_total_rows(sheet_name=self.sheet2Name)
        modbusData = []
        ...
        for device in self.device_name_list:
            ...
            device_data_start_index = device[1] -2 # point to the start row index of data for a device inside the array
            device_data_end_index = device[2] - 1  # point to the end row of data for a device inside the array  
    ...
    def reorderData(self):
        ...
        for device in self.device_name_list:
            ...
            for data in device_data:
                sorted_obj = self.change_order(data, ["measure-id"] + self.desiredColumns)
...
if __name__ == "__main__":
    ...
    desColumns = ['Codice Stabilimento', 'Descrizione Stabilimento', 'Edificio', 'Vettore', 'POD', 'Piano', 'Reparto', 'Quadro', 'Descrizione Quadro', 'Sottoquadro', 'Descrizione Sottoquadro', 'Linea', 'Descrizione Linea', 'Area Funzionale ENEA', 'Cod. Funzionale TERA', 'Tipologia Dispositivo', 'Codice Modello Dispositivo', 'Taglia Interruttore', 'Tipologia Misura', 'Hostname Edge', 'ID Modbus', 'Tipo Dispositivo', 'mqtt-topic', 'modbus-id', 'modbus-fc', 'modbus-address', 'modbus-format', 'Misura', 'Unita di misura', 'modbus-n_registers', 'mqtt-qos', 'group', 'ime-type']
    builder = CsvBuilder("Hostname Edge","sample.xlsx","Plant","Modbus",desColumns)
